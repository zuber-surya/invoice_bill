...
import { Party, Item, Estimation, InvoiceItem, BusinessSettings } from '../types';
...
import { EstimationPreview } from './EstimationPreview';
...
interface EstimationFormProps {
  ...
  onSuccess: () => void;
  initialData?: Partial<Estimation>;
}

export function EstimationForm({ parties, items, settings, onSuccess, initialData }: EstimationFormProps) {
  const [selectedParty, setSelectedParty] = useState<Party | null>(initialData?.partyDetails || null);
  const [estimationDate, setEstimationDate] = useState(initialData?.date || format(new Date(), 'yyyy-MM-dd'));
  const [expiryDate, setExpiryDate] = useState(initialData?.expiryDate || format(addDays(new Date(), 15), 'yyyy-MM-dd'));
  const [estimationNumber, setEstimationNumber] = useState(initialData?.estimationNumber || '');
  ...
  useEffect(() => {
    if (settings && !initialData?.estimationNumber) {
      setEstimationNumber(`EST-${Date.now().toString().slice(-6)}`);
    }
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const estimationData: Omit<Estimation, 'id'> = {
        estimationNumber,
        date: estimationDate,
        expiryDate,
  ...
  const exportImage = async () => {
    ...
    link.download = `${estimationNumber}.png`;
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500">Generate a quote or proforma invoice</p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) => setEstimationNumber(e.target.value)}
